...
- **No look on the camera page**: the Natural chip and the looks row it
  ...
  them.
- **Chat: a camera, a gallery, and the photo's set pieces**: the
  composer has a camera button (the system camera takes a picture that
  lands in the composer) and a gallery button (the picker brings one
  in). Two pills are always on offer above the composer - *Translate* (Gemma reads and translates
  ...
  Gemma cleans it - or the reading stands when the model is away). With
  a photo attached they act on it; without, the camera opens first and
